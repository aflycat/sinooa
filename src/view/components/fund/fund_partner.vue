...
<!-- 基金合伙人信息 -->
    <div class="found_partner">
                    ...
                    <Row>
                           <Col span="8">
                            <FormItem label="基金简称：" prop="ProjectType">
                                <Select v-model="submitData.ProjectID" filterable  placeholder="请选择基金" @on-change="selectfundDetail">
           ...
           <pro-inform :objId="projectID" :flag="3"></pro-inform>
    ...
    data(){
        return{
            ...
            fileForm:new FormData(),
             submitData:{
                    TaskTypeID:TaskTypeID.fundPartnership,//任务类别ID，与TaskTypes表的TaskTypeID对应（开发3/立项4/变动5），取自对应的菜单项
        ...
        getProList(status){
            ...
            getProjectList({"ProjectStatus":2,"USerID":JSON.parse(localStorage.getItem("userId"))}).then(res=>{
                if(res.data.code==2306){
                    ...
                    this.$Message.error({
                        content:'基金列表数据加载失败：'+res.data.message
                    })
                }
            })
        },
