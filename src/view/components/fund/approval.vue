...
import {getPlatform,getuserList,
addNewFundTask,getAllFundList,getFundDetail,addNewFundTaskFile,uploadFile,clientListQuery
} from "@/api/data"
export default {
    components:{
        UploadFiles
            ...
            getFundDetail({FundID:this.fundID}).then(res=>{
                if(res.data.code==2406){
                    this.postdata={
                        TaskName:'',//任务名（UI中的请示事项要点）
                        TaskSummary:'',//任务概要（UI中的请示事项具体内容）
                        TaskTypeID:TaskTypeID.fundProjects,
